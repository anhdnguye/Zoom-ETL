...
class DataLoader:
    ...
    def load_recordings(self, recordings: List[Dict[str, Any]]):
        """Load recording data in batches."""
        if not recordings:
            self.logger.info("No recordings to load")
            return

        query = """
            INSERT INTO recording (
                meeting_uuid, file_id, file_type, file_size, file_extension,
                recording_start, recording_end, recording_type, download_url, file_path
            )
            VALUES %s
        ...
        for i in range(0, len(recordings), self.batch_size):
            ...
            data = [
                (
                    recording.get('meeting_uuid'),
                    recording.get('file_id'),
                    recording.get('file_type'),
                    recording.get('file_size'),
                    recording.get('file_extension'),
                    recording.get('recording_start'),
                    recording.get('recording_end'),
            ...
            self._execute_batch(query, data, "recording")